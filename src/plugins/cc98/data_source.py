...
class MyCC98Api(CC98_API_V2):
    @run_sync
    def get_board_name(self, name: str) -> Tuple[str, float]:
    ...
    @run_sync
    def get_board_id(self, name: str) -> int:
        board_all = self.board_all2()
    ...
    @run_sync
    def get_topics(self, board_name: str):
        if board_name == '十大':
    ...
    @run_sync
    def print_topics(self, topics: List[dict]) -> List[str]:
        msgs = []
        for topic in topics:
    ...
    @run_sync
    def print_posts(self, topic: dict, page: int) -> List[str]:
        msgs = []
        board_name = self.board(topic['boardId'])['name']
    ...
    @run_sync
    def replace_url(self, url: str) -> Union[str, MessageSegment]:
    ...
    @run_sync
    @staticmethod
    def replace_emoji(emoji: str) -> Union[str, MessageSegment]:
        emoji = emoji.lower()
        dir_name = ''
    ...
    @run_sync
    def simplify_content(self, s: str):
        s = s.replace(
