...
cc98_api = CC98_API_V2(
    cc98_config.cc98_user_name, cc98_config.cc98_user_password, httpx_proxy
)
...
async def print_posts(topic: dict, page: int) -> List[str]:
    ...
    msgs.append(f"[{board_name}]{topic['title']}\n[page: {page}/{page_num}]")
...
async def simplify_content(s: str):
    ...
    while re.findall(quote_user, s, flags=re.S):
        s = re.sub(
            quote_user,
            lambda x: ("# 引用了" + x.group(1) + "楼的发言 #" + x.group(2) + line),
            s,
            flags=re.S,
        )
    ...
    while re.findall(quote_content, s, flags=re.S):
        s = re.sub(
            quote_content,
            lambda x: (" # 以下为引用内容：# " + x.group(1) + line),
            s,
            flags=re.S,
        )
    ...
    s = re.sub(
        r"\[bili](.*?)\[/bili]",
        lambda x: "https://www.bilibili.com/video/av" + x.group(1),
        s,
        flags=re.S,
    )
    ...
    for p in img_patterns:
        s = re.sub(p, lambda x: f"##img##{x.group(1)}##/img##", s, flags=re.S)
    ...
    for p in emoji_patterns:
        s = re.sub(p, lambda x: f"##emoji##{x.group(1)}##/emoji##", s, flags=re.S)
