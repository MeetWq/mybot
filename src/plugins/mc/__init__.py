...
async def bind(matcher: Matcher, user_id: str, url: str, **kwargs):
    ...
    await matcher.finish(
        "成功绑定动态地图，使用“dynmap chaton/chatoff”开启或关闭聊天转发；使用“dynmap status”查看在线状态"
    )
...
async def status(matcher: Type[Matcher], user_id: str, **kwargs):
...
dynmap = on_shell_command("dynmap", parser=dynmap_parser, block=True, priority=15)
